...
void StatsWidget::OBSFrontendEvent(enum obs_frontend_event event, void *ptr)
{
	StatsWidget *stats = reinterpret_cast<StatsWidget *>(ptr);
...
StatsWidget::StatsWidget(QWidget *parent, bool closeable)
	: QWidget(parent),
	  timer(this),
	  recTimeLeft(this)
{
...
void StatsWidget::AddOutputLabels(QString name)
{
	OutputLabels ol;
...
void StatsWidget::InitializeValues()
{
...
void StatsWidget::StartRecTimeLeft()
{
	recordTimeLeft->setText(QTStr("Calculating"));
	recTimeLeft.start();
}

void StatsWidget::ResetRecTimeLeft()
{
	bitrates.clear();
	recTimeLeft.stop();
...
void StatsWidget::RecordingTimeLeft()
{
...
void StatsWidget::OutputLabels::Update(obs_output_t *output, bool rec)
{
...
void StatsWidget::OutputLabels::Reset(obs_output_t *output)
{
	if (!output)
		return;
...
void StatsWidget::hideEvent(QHideEvent *)
{
	timer.stop();
}
